...
class DeliveryTruckTest {
	...
	@Test
	void testCountCubes() {
		assertEquals(1, t1.countCubes());
		assertEquals(6, t2.countCubes());
		assertEquals(0, t3.countCubes());
	}
	...
	@Test
	void testGetWithVolume() {
		Box result = t1.getWithVolume(162);
		assertNotNull(result);
		assertNotEquals(result, c); //not reference copy
		assertEquals(2, result.depth);
		assertEquals(9, result.height);
		assertEquals(9, result.width);

		result = t1.getWithVolume(12);
		assertNull(result);

		result = t2.getWithVolume(162);
		assertNotNull(result);
		assertNotEquals(result, d); //not reference copy
		assertEquals(18, result.depth);
		assertEquals(3, result.height);
		assertEquals(3, result.width);

		result = t1.getWithVolume(6);
		assertNotNull(result);
