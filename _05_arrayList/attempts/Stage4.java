...
public class Stage4 {
	...
	public static void sort(ArrayList<Integer> list) {
		...
		for (int i=0;i<list.size()-1;i++) {
			int minIndex = list.get(i); //store item at i
			
			for(int k=i+1;k<list.size()-2;k++) {
				if(list.get(k) < list.get(minIndex)) {// if item after i is smaller than i
					minIndex = list.get(k); //store k
				}
			}
			//swap values
			int store = list.get(i); // item i
			list.set(i, minIndex); //replace item i with item k
			list.set(minIndex, store); // replace item k with item i
		}
	}
